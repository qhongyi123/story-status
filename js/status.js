...
function buildSectionTitle(iconEntity, title) {
    var t = $('<div>').addClass('section-title');
    t.html(iconEntity + ' ' + title);
    return t;
}

function buildKVList(rows) {
    var list = $('<div>').addClass('kv-list');
    var has = false;
    rows.forEach(function(r) {
        var v = fmtValue(r.value);
        if (!v) return;
        has = true;
        var row = $('<div>').addClass('kv-row');
        var lbl = $('<span>').addClass('kv-label');
        lbl.text(r.label + '：');
        var val = $('<span>').addClass('kv-value');
        val.text(v);
        row.append(lbl);
        row.append(val);
        list.append(row);
...
function buildEmptyHint(text) {
    var hint = $('<div>').addClass('empty-hint');
    hint.text(text || '暂无数据...');
    return hint;
}
...
function buildEntityCard(name, rows) {
    ...
    var nameEl = $('<div>').addClass('entity-name');
    nameEl.text(name);
    card.append(nameEl);
...
function collectEstateRevenue(name, estate, todayISO, currentWealth, btn, info) {
    ...
    if (info) info.text('上次收款：' + todayISO);
    if (btn) { btn.text('已收 ' + amount + ' 银币'); btn.prop('disabled', true); }
}
...
function collectEstateHarvest(name, estate, todayISO, warehouse, region, btn, info) {
    ...
    if (info) info.text('上次收获：' + todayISO);
    if (btn) { btn.text('已收 ' + Math.round(totalGain) + ' 磅'); btn.prop('disabled', true); }
}
...
function collectSell(name, estate, todayISO, currentWealth, warehouse, region, btn, info) {
    ...
    if (btn) { btn.text('已卖 ' + total + ' 银币'); btn.prop('disabled', true); }
    if (info) info.text('售出：' + items.join('、'));
}
...
function buildWarehouseTable(regionWarehouse) {
    ...
    var table = $('<table>').addClass('cargo-table');
    var thead = $('<thead>');
    var hr = $('<tr>');
...
function buildWarehouseBlock(warehouse, onlyRegion) {
    ...
    var block = $('<div>').addClass('warehouse-block');
    regions.forEach(function(region) {
        var title = $('<div>').addClass('entity-group-title');
        title.text('仓库 · ' + region);
        block.append(title);
        block.append(buildWarehouseTable(warehouse[region]));
    });
    return block;
}
...
function buildEstateCard(name, estate, todayISO, currentWealth, warehouse, staffCount) {
    ...
    if (outInfo) {
        var outputWrap = $('<div>').addClass('output-block');
        var outputRow = $('<div>').addClass('output-row');
        var ol = $('<span>').addClass('output-label');
        ol.text(outInfo.label + '：');
        var ov = $('<span>').addClass('output-value');
        ov.text(outInfo.value);
        outputRow.append(ol);
        outputRow.append(ov);
        outputWrap.append(outputRow);
        card.append(outputWrap);
    }

    /* 就职人员（软门控提示） */
    if (gatedType) {
        var staffWrap = $('<div>').addClass('staff-row');
        var staffLabel = $('<span>').addClass('staff-label');
        staffLabel.text('就职：' + (staffCount || 0) + ' 人');
        staffWrap.append(staffLabel);
        if (blocked) {
            var staffHint = $('<span>').addClass('staff-hint');
            staffHint.text('无人经营，无法操作');
            staffWrap.append(staffHint);
        }
        card.append(staffWrap);
    }

    /* 收款（商业/手工业 revenue） */
    if (estate.revenue) {
        var wrap = $('<div>').addClass('collect-row');
        var btn = $('<button>').attr('type', 'button').addClass('collect-btn');
        btn.text('收款');
        if (blocked || (estate.last_collected && estate.last_collected === todayISO)) {
            btn.prop('disabled', true);
        }
        var info = $('<span>').addClass('collect-info');
        info.text(estate.last_collected ? ('上次收款：' + estate.last_collected) : '');
    ...
    /* 收获（农事） */
    if (estate.output) {
        var hwrap = $('<div>').addClass('collect-row');
        var hbtn = $('<button>').attr('type', 'button').addClass('collect-btn');
        hbtn.text('收获');
        var hinfo = $('<span>').addClass('collect-info');
        hinfo.text(estate.last_harvested ? ('上次收获：' + estate.last_harvested) : '');
        if (blocked) hbtn.prop('disabled', true);
        if (estate.last_harvested) {
            var hdays = daysBetween(estate.last_harvested, todayISO);
            if (!isNaN(hdays) && hdays < HARVEST_MIN_DAYS) hbtn.prop('disabled', true);
        }
    ...
    if (estate.recipes && estate.recipes.length) {
        var rwrap = $('<div>').addClass('recipe-block');
        estate.recipes.forEach(function(recipe) {
            var row = $('<div>').addClass('recipe-row');
            var rlabel = $('<span>').addClass('recipe-label');
            rlabel.text(fmtRecipe(recipe));
            row.append(rlabel);
            var rinput = $('<input>').attr('type', 'number').attr('min', '1').val('1').addClass('recipe-batch');
            row.append(rinput);
            var rbtn = $('<button>').attr('type', 'button').addClass('collect-btn');
            rbtn.text('转化');
            if (blocked) rbtn.prop('disabled', true);
            rbtn.on('click', function() {
                collectConversion(name, estate, todayISO, warehouse, region, recipe, rinput.val(), rbtn);
            });
            row.append(rbtn);
            rwrap.append(row);
        });
    ...
    if (estate.type === '商业') {
        var swrap = $('<div>').addClass('collect-row');
        var sbtn = $('<button>').attr('type', 'button').addClass('collect-btn');
        sbtn.text('出售该区商品');
        if (blocked) sbtn.prop('disabled', true);
        var sinfo = $('<span>').addClass('collect-info');
...
function buildShipCard(name, ship) {
    ...
    if (Object.keys(cargo).length) {
        var cargoHeader = $('<div>').addClass('cargo-header');
        var cargoLabel = $('<span>').addClass('cargo-label');
        cargoLabel.text('货物：');
        cargoHeader.append(cargoLabel);

        var btnOverview = $('<button>').attr('type', 'button').addClass('cargo-mode-btn').addClass('active');
        btnOverview.text('总览');

        var btnCategory = $('<button>').attr('type', 'button').addClass('cargo-mode-btn');
        btnCategory.text('分类');

        cargoHeader.append(btnOverview);
        cargoHeader.append(btnCategory);
        card.append(cargoHeader);

        var cargoContent = $('<div>').addClass('cargo-content');
        card.append(cargoContent);

        function renderCargo(mode) {
            cargoContent.empty();
            if (mode === 'category') {
                cargoContent.append(buildCargoByCategory(cargo));
            } else {
                cargoContent.append(buildCargoOverview(cargo));
            }
            btnOverview.toggleClass('active', mode === 'overview');
            btnCategory.toggleClass('active', mode === 'category');
        }
        btnOverview.on('click', function() { renderCargo('overview'); });
        btnCategory.on('click', function() { renderCargo('category'); });
        renderCargo('overview');
    }

    /* 货物估价按钮（方法 A：按分类加权品质插值计算货值） */
    var btn = $('<button>').attr('type', 'button').addClass('cargo-valuate-btn');
    btn.text('点击对货物估价');
    btn.on('click', function() {
        if (btn.hasClass('valued')) {
            btn.removeClass('valued');
            btn.text('点击对货物估价');
        } else {
            btn.addClass('valued');
            btn.text('估值：' + calculateCargoValue(cargo) + ' 银币');
        }
    });
    card.append(btn);
...
function buildPersonCard(name, person, employment, data) {
    ...
    card.attr('data-person-name', name);
    if (SELECTED_PERSON === name) card.addClass('person-selected');
    var nameEl = card.find('.entity-name');
    if (nameEl.length) {
        nameEl.addClass('entity-name-with-symbol');
        var sym = $('<span>').addClass('entity-gender-symbol');
        sym.text(genderSymbol(person.gender));
        nameEl.append(sym);
    }
    var a = employment && employment[name];
    var line = $('<div>').addClass('assign-row');
    var lbl = $('<span>').addClass('assign-label');
    lbl.text('就职：');
    line.append(lbl);
    var val = $('<span>').addClass('assign-value');
    val.text(a && a.name ? ((a.type === 'estate' ? '家产 · ' : '船只 · ') + a.name) : '未就职');
    line.append(val);
    card.append(line);
    ...
    card.on('click', function() {
        ...
        if (SELECTED_PERSON === name) {
            ...
            card.removeClass('person-selected');
            closeCommandPanel();
        } else {
            $('.entity-card.person-selected').removeClass('person-selected');
            SELECTED_PERSON = name;
            card.addClass('person-selected');
...
function buildPayrollTable(rel, employment) {
    ...
    var table = $('<table>').addClass('payroll-table');
    var thead = $('<thead>');
    var hr = $('<tr>');
...
function buildUnassignedPersonSelect(data) {
    var sel = $('<select>').addClass('assign-select');
    var none = $('<option>');
    none.val('');
    none.text('选择人员…');
    sel.append(none);
...
function buildEmploymentCardBody(targetType, targetName, staffList, data, rerender) {
    var body = $('<div>').addClass('assign-popover-body');

    if (staffList && staffList.length) {
        staffList.forEach(function(p) {
            var row = $('<div>').addClass('staff-person-row');
            var label = $('<span>').addClass('staff-person-name');
            label.text(p.name);
            row.append(label);
            var unBtn = $('<button>').attr('type', 'button').addClass('collect-btn');
            unBtn.text('解职');
            unBtn.on('click', function() { emitUnassign(p, data, rerender); });
            row.append(unBtn);
            body.append(row);
        });
    } else {
        var none = $('<div>').addClass('staff-hint');
        none.text('无人就职');
        body.append(none);
    }

    var ctrl = $('<div>').addClass('assign-ctrl');
    var select = buildUnassignedPersonSelect(data);
    ctrl.append(select);
    var goBtn = $('<button>').attr('type', 'button').addClass('collect-btn');
    goBtn.text('就职');
...
function openAssignPopover(targetType, targetName, staffList, data, rerender) {
    closeAssignPopover();
    var anchor = $('.asset-tile.selected');
    if (!anchor.length) return;

    var popover = $('<div>').addClass('assign-popover');

    var head = $('<div>').addClass('assign-popover-head');
    var title = $('<span>').addClass('assign-popover-title');
    title.text((targetType === 'estate' ? '家产 · ' : '船只 · ') + targetName);
    head.append(title);
    var closeBtn = $('<button>').attr('type', 'button').addClass('assign-popover-close');
    closeBtn.text('✕');
    closeBtn.on('click', function() {
        CURRENT_ASSIGN_TARGET = null;
        closeAssignPopover();
        $('.asset-tile.selected').removeClass('selected');
    });
    head.append(closeBtn);
    popover.append(head);

    popover.append(buildEmploymentCardBody(targetType, targetName, staffList, data, rerender));
    ...
    popover.css('left', (rect.left + sx) + 'px');
    popover.css('top', (rect.bottom + 6 + sy) + 'px');
}
...
function buildAssetBoard(type, names, assetMap, ai, data, rerender) {
    ...
    var board = $('<div>').addClass('asset-board');

    var gridEl = $('<div>').addClass('asset-grid');
    var tiles = [];
    layout.forEach(function(it) {
        ...
        var cell = $('<div>').addClass('asset-tile').addClass(cls);
        cell.css('gridColumn', (it.col + 1) + ' / span ' + it.w);
        cell.css('gridRow', (it.row + 1) + ' / span ' + it.h);
        cell.text(it.name);
...
function paySalaries(monthlyTotal, todayISO, currentWealth, payroll, btn) {
    ...
    if (btn) { btn.text('已发 ' + monthlyTotal + ' 银币'); btn.prop('disabled', true); }
}
...
function renderManagementTab(data, content, rerender) {
    ...
    var payBlock = $('<div>').addClass('payroll-block');
    var payTitle = $('<div>').addClass('entity-group-title');
    payTitle.text('薪资');
    payBlock.append(payTitle);
    var payInfo = $('<div>').addClass('payroll-info');
    payInfo.text('本月应发：' + monthlyTotal + ' 银币' + (payroll.last_paid ? '（上次发薪 ' + payroll.last_paid + '）' : ''));
    payBlock.append(payInfo);
    var payRow = $('<div>').addClass('collect-row');
    var payBtn = $('<button>').attr('type', 'button').addClass('collect-btn');
    payBtn.text('发薪');
    if (!monthlyTotal || monthlyTotal <= 0 || payroll.last_paid === todayISO) payBtn.prop('disabled', true);
    payBtn.on('click', function() {
        paySalaries(monthlyTotal, todayISO, currentWealth, payroll, payBtn);
    });
    payRow.append(payBtn);
    payBlock.append(payRow);
    content.append(payBlock);

    /* 薪资明细（折叠，点击展开） */
    var payDetail = $('<div>').addClass('payroll-detail');
    var payDetailHeader = $('<div>').addClass('payroll-detail-header');
    payDetailHeader.text('薪资明细 ▸');
    var payDetailBody = $('<div>').addClass('payroll-detail-body');
    payDetailBody.css('display', 'none');
    payDetailBody.append(buildPayrollTable(rel, data.employment || {}));
    payDetailHeader.on('click', function() {
        var collapsed = payDetailBody.css('display') === 'none';
        payDetailBody.css('display', collapsed ? '' : 'none');
        payDetailHeader.text('薪资明细 ' + (collapsed ? '▾' : '▸'));
    });
    payDetail.append(payDetailHeader);
    payDetail.append(payDetailBody);
    content.append(payDetail);

    var assignTitle = $('<div>').addClass('entity-group-title');
    assignTitle.text('人员分配');
    content.append(assignTitle);
    ...
    /* 家产/船只 选项卡 */
    var tabBar = $('<div>').addClass('entity-tabs');
    [['家产', estateNames.length], ['船只', shipNames.length]].forEach(function(bt) {
        var btn = $('<button>').attr('type', 'button').addClass('entity-tab' + (bt[0] === CURRENT_BOARD_TAB ? ' active' : ''));
        btn.text(bt[0] + bt[1]);
        btn.attr('data-tab', bt[0]);
        tabBar.append(btn);
    });
    ...
    function appendBoard(type, names, assetMap) {
        ...
        content.append(result.board);
        allTiles = allTiles.concat(result.tiles);
        if (typeof ResizeObserver !== 'undefined') {
            var ro = new ResizeObserver(function() { result.tiles.forEach(fitTileText); });
            ro.observe(result.gridEl[0]);
        }
    }
    if (CURRENT_BOARD_TAB === '家产') {
        if (estateNames.length) {
            /* 按地区分类勾选项 */
            var regionToggle = $('<label>').addClass('region-toggle');
            var cb = $('<input>').attr('type', 'checkbox');
            cb.prop('checked', GROUP_BY_REGION);
            cb.on('change', function() {
                GROUP_BY_REGION = cb.prop('checked');
                rerender();
            });
            regionToggle.append(cb);
            regionToggle.append('按地区分类');
            content.append(regionToggle);
            ...
            if (GROUP_BY_REGION) {
                ...
                orderedRegions.forEach(function(region) {
                    var regionTitle = $('<div>').addClass('entity-group-title');
                    regionTitle.text(region);
                    content.append(regionTitle);
            ...
            content.append(buildEmptyHint('暂无家产...'));
        }
    } else {
        if (shipNames.length) appendBoard('ship', shipNames, data.ships || {});
        else content.append(buildEmptyHint('暂无船只...'));
    }
    ...
    tabBar.on('click', function(e) {
        ...
        if (CURRENT_BOARD_TAB === btn.attr('data-tab')) return;
        CURRENT_BOARD_TAB = btn.attr('data-tab');
        CURRENT_ASSIGN_TARGET = null;
        rerender();
    });
...
function renderRelatedPersonsTab(data, content) {
    ...
    if (!orderedTags.length) { content.append(buildEmptyHint('暂无相关人员...')); return; }
    orderedTags.forEach(function(tag) {
        var names = groups[tag];
        var grp = $('<div>').addClass('entity-group');
        var header = $('<div>').addClass('entity-group-title').addClass('rel-group-header');
        var nameSpan = $('<span>').addClass('rel-group-name');
        nameSpan.text(tag);
        var arrow = $('<span>').addClass('rel-arrow');
        arrow.text('▾');
        nameSpan.append(arrow);
        var countSpan = $('<span>').addClass('rel-count');
        countSpan.text(names.length);
        header.append(nameSpan);
        header.append(countSpan);
        var body = $('<div>').addClass('rel-group-body');
        names.forEach(function(n) { body.append(buildPersonCard(n, rel[n] || {}, employment, data)); });
...
function openCommandPanel(data, anchorEl) {
    ...
    var popover = $('<div>').addClass('command-popover');

    var head = $('<div>').addClass('command-popover-head');
    var title = $('<span>').addClass('command-popover-title');
    title.text('指令');
    head.append(title);
    var closeBtn = $('<button>').attr('type', 'button').addClass('assign-popover-close');
    closeBtn.text('✕');
    closeBtn.on('click', closeCommandPanel);
    head.append(closeBtn);
    popover.append(head);

    var target = $('<div>').addClass('command-target');
    target.text('目标角色：' + (SELECTED_PERSON || '未选择（请在相关人员里点击角色）'));
    popover.append(target);

    var favorites = Object.keys(usage).filter(function(n) { return usage[n] >= 3; });
    if (favorites.length) {
        var favWrap = $('<div>').addClass('command-fav');
        var favTitle = $('<div>').addClass('command-fav-title');
        favTitle.text('常用角色');
        favWrap.append(favTitle);
        favorites.forEach(function(n) {
            var chip = $('<button>').attr('type', 'button').addClass('command-fav-chip' + (SELECTED_PERSON === n ? ' selected' : ''));
            chip.text(n);
            chip.on('click', function() {
                SELECTED_PERSON = n;
                target.text('目标角色：' + n);
                favWrap.find('.command-fav-chip').removeClass('selected');
                chip.addClass('selected');
                $('.person-card.person-selected').removeClass('person-selected');
                var pc = $('.person-card[data-person-name="' + n + '"]');
                if (pc.length) pc.addClass('person-selected');
            });
            favWrap.append(chip);
        });
        popover.append(favWrap);
    }

    COMMAND_GROUPS.forEach(function(grp) {
        var g = $('<div>').addClass('command-group');
        var gTitle = $('<div>').addClass('command-group-title');
        gTitle.text(grp.name);
        g.append(gTitle);
        grp.commands.forEach(function(cmd) {
            var btn = $('<button>').attr('type', 'button').addClass('command-item');
            btn.text(cmd.name);
            btn.on('click', function() {
    ...
    if (anchorEl) {
        ...
        var cardEl = $('.status-card');
        var cardRect = cardEl.length ? cardEl[0].getBoundingClientRect() : { left: 0, right: $(window).width() };
        var popWidth = 320;
        var sx = $(window).scrollLeft();
        var sy = $(window).scrollTop();
        var left = Math.max(cardRect.left + 8, Math.min(rect.left, cardRect.right - popWidth - 8));
        popover.css('left', (left + sx) + 'px');

        /* 先放下方，若触底则改放上方 */
        var top = rect.bottom + 6 + sy;
        var popHeight = popover.outerHeight();
        if (popHeight && top + popHeight > sy + $(window).height()) {
            top = rect.top - 6 + sy - popHeight;
            if (top < sy + 8) top = sy + 8;
        }
        popover.css('top', top + 'px');
    } else {
        popover.css('right', '20px');
        popover.css('top', '80px');
    }
}
...
var SECTION_RENDERERS = {
    ...
    estate: function(data, container) {
        ...
        function render() {
            section.empty();
            section.append(buildSectionTitle('&#x1F3E0;', '家产'));
            section.append(buildWarehouseBlock(warehouse, CURRENT_CONTINENT));

            /* 大洲切换栏 */
            var contBar = $('<div>').addClass('entity-tabs');
            ordered.forEach(function(c) {
                var btn = $('<button>').attr('type', 'button').addClass('entity-tab' + (c === CURRENT_CONTINENT ? ' active' : ''));
                btn.text(c + continentCount(c));
                btn.attr('data-continent', c);
                contBar.append(btn);
            });
            section.append(contBar);
            ...
            ESTATE_TYPES.forEach(function(et) {
                var btn = $('<button>').attr('type', 'button').addClass('entity-tab' + (et.label === CURRENT_ESTATE_TAB ? ' active' : ''));
                btn.text(et.label + (groups[et.label] ? groups[et.label].length : 0));
                btn.attr('data-type', et.label);
                tabBar.append(btn);
            });
            section.append(tabBar);

            /* 内容区 */
            var content = $('<div>').addClass('entity-tab-content');
            ...
            function renderContent() {
                ...
                content.empty();
                if (!list.length) {
                    content.append(buildEmptyHint('暂无' + CURRENT_CONTINENT + '·' + CURRENT_ESTATE_TAB + '类家产...'));
                    return;
                }
                list.forEach(function(item) {
                    content.append(buildEstateCard(item.name, item.data, todayISO, currentWealth, warehouse, staffIndex[item.name] || 0));
                });
            }
            renderContent();

            /* 大洲切换 */
            contBar.on('click', function(e) {
                var btn = $(e.target).closest('.entity-tab');
                if (!btn.length) return;
                if (CURRENT_CONTINENT === btn.attr('data-continent')) return;
                CURRENT_CONTINENT = btn.attr('data-continent');
                render();
            });
            ...
            tabBar.on('click', function(e) {
                var btn = $(e.target).closest('.entity-tab');
                if (!btn.length) return;
                CURRENT_ESTATE_TAB = btn.attr('data-type');
                tabBar.find('.entity-tab').each(function() {
                    $(this).toggleClass('active', this === btn[0]);
                });
                renderContent();
            });
        }
    ...
    ships: function(data, container) {
        ...
        var section = $('<div>').addClass('section');
        section.append(buildSectionTitle('&#x1F6A2;', '船只'));
        ...
        if (!Object.keys(ships).length) {
            section.append(buildEmptyHint('暂无船只...'));
            container.append(section);
            return;
        }
        ...
        tabTypes.forEach(function(t) {
            var btn = $('<button>').attr('type', 'button').addClass('entity-tab' + (t === CURRENT_SHIP_TAB ? ' active' : ''));
            btn.text(t + (groups[t] ? groups[t].length : 0));
            btn.attr('data-type', t);
            tabBar.append(btn);
        });
        section.append(tabBar);
        ...
        function renderContent() {
            ...
            content.empty();
            if (!list.length) {
                content.append(buildEmptyHint('暂无' + CURRENT_SHIP_TAB + '类船只...'));
                return;
            }
            list.forEach(function(item) {
                content.append(buildShipCard(item.name, item.data));
            });
        }
        renderContent();
        ...
        tabBar.on('click', function(e) {
            var btn = $(e.target).closest('.entity-tab');
            if (!btn.length) return;
            CURRENT_SHIP_TAB = btn.attr('data-type');
            tabBar.find('.entity-tab').each(function() {
                $(this).toggleClass('active', this === btn[0]);
            });
            renderContent();
    ...
    relationship: function(data, container) {
        ...
        function render() {
            section.empty();
            section.append(buildSectionTitle('&#x1F465;', '关系'));

            var topRow = $('<div>').addClass('rel-top-row');
            var tabBar = $('<div>').addClass('entity-tabs');
            ['管理', '相关人员'].forEach(function(t) {
                var btn = $('<button>').attr('type', 'button').addClass('entity-tab' + (t === CURRENT_REL_TAB ? ' active' : ''));
                btn.text(t);
                btn.attr('data-tab', t);
                tabBar.append(btn);
            });
            topRow.append(tabBar);
            var cmdToggle = $('<label>').addClass('command-toggle');
            var cmdCb = $('<input>').attr('type', 'checkbox');
            cmdCb.prop('checked', COMMAND_MODE);
            cmdCb.on('change', function() {
                COMMAND_MODE = cmdCb.prop('checked');
                if (!COMMAND_MODE) {
                    SELECTED_PERSON = null;
                    closeCommandPanel();
                    $('.person-card.person-selected').removeClass('person-selected');
            ...
            cmdToggle.append(cmdCb);
            cmdToggle.append('指令');
            topRow.append(cmdToggle);
            ...
            tabBar.on('click', function(e) {
                var btn = $(e.target).closest('.entity-tab');
                if (!btn.length) return;
                if (CURRENT_REL_TAB === btn.attr('data-tab')) return;
                CURRENT_REL_TAB = btn.attr('data-tab');
                render();
            });
        }
        render();
    ...
    region: function(data, container) {
        ...
        var section = $('<div>').addClass('section');
        section.append(buildSectionTitle('&#x1F5FA;', '地区'));
        var names = Object.keys(region);
        if (!names.length) { section.append(buildEmptyHint('暂无地区信息...')); }
        else names.forEach(function(name) {
            var r = region[name] || {};
            section.append(buildEntityCard(name, [
...
/* =====================================================================
 * 入口：等待 Mvu 框架初始化后读取变量并渲染
 * ===================================================================== */
$(() => {
    errorCatched(function() {
        var App = {
            state: {
            ...
            uiStateConfig: {
                ...
                applyCollapse: function() {
                    if (this.data.isPanelCollapsed) {
                        App.elements.statusCard.addClass('global-collapsed');
                        setTimeout(function() {
                            $('body').css('paddingTop', '11px');
                            $('body')[0].offsetHeight;
                            $('body').css('paddingTop', '');
                        }, 30);
                    } else {
                        App.elements.statusCard.removeClass('global-collapsed');
                    }
                }
            },
            ...
            elements: {
                ...
                collapsePanelBtn: $('#collapse-panel-btn'),
                avatar: $('#avatar'),
                avatarPlaceholder: $('#avatar-placeholder'),
                text: {
                    title: $('#char-title'),
                    psyche: $('#psyche-bubble'),
                    tagContainer: $('#identity-tags-container')
                },
                containers: {
                    ...
                    stageCollapseToggle: $('#stage-collapse-toggle-area'),
                    headerBtnArea: $('#header-stage-btn-area'),
                    userHeader: $('#user-header'),
                    modeSections2: $('#mode-sections-2'),
                    modeSections3: $('#mode-sections-3'),
                    modeSections4: $('#mode-sections-4'),
                    modeSections5: $('#mode-sections-5'),
                    modeSections6: $('#mode-sections-6'),
                    worldInfo: $('#world-info'),
                    worldInfoTop: $('#wib-top'),
                    worldInfoExpand: $('#wib-expand'),
                    pages: $('.page')
                },
                settings: {
                    toggle: $('#settings-toggle'),
                    panel: $('#settings-panel'),
                    sliders: { width: $('#width-slider'), spacing: $('#spacing-slider'), fontSize: $('#font-size-slider'), avatarSize: $('#avatar-size-slider') },
                    values: { width: $('#width-value'), spacing: $('#spacing-value'), fontSize: $('#font-size-value'), avatarSize: $('#avatar-size-value') },
                    fontSelector: $('#font-selector'),
                    buttons: { reset: $('#reset-settings-btn') }
                },
                deleteConfirm: { panel: $('#delete-confirm-panel'), msgContent: $('#delete-msg-content'), btnConfirm: $('#confirm-delete-btn'), btnCancel: $('#cancel-delete-btn') },
                itemDetail: {
                    panel: $('#item-detail-panel'),
                    nameElem: $('#detail-item-name'),
                    descElem: $('#detail-item-description'),
                    btnDelete: $('#detail-delete-btn'),
                    btnClose: $('#detail-close-btn')
                },
                pageTurnBtn: $('#page-turn-bookmark')
            },
            ...
            settings: {
                ...
                apply: function(s) {
                    ...
                    requestAnimationFrame(function() {
                        e.root.css('--card-max-width', s.width + 'px');
                        e.root.css('--section-gap', s.spacing + 'rem');
                        e.root.css('--base-font-size', s.fontSize + 'px');
                        e.root.css('--avatar-size', s.avatarSize + 'px');
                        e.root.css('--font-main', s.fontFamily);
                        Object.keys(e.settings.values).forEach(function(key) {
                            if (e.settings.values[key] && e.settings.values[key].length) {
                                var val = s[key]; var unit = key.indexOf('spacing') !== -1 ? 'rem' : 'px';
                                e.settings.values[key].text(val + unit);
                            }
                        });
                    });
                },
                updateUIControls: function(s) {
                    Object.keys(App.elements.settings.sliders).forEach(function(k) { if (App.elements.settings.sliders[k].length) App.elements.settings.sliders[k].val(s[k]); });
                    App.elements.settings.fontSelector.val(s.fontFamily);
                },
                save: function() {
                    ...
                    Object.keys(sliders).forEach(function(k) { if(sliders[k].length) newS[k] = parseFloat(sliders[k].val()); });
                    newS.fontFamily = fontSelector.val();
            ...
            ui: {
                updateAvatar: function(file) {
                    ...
                    if (file) {
                        ...
                        App.elements.avatar.css('backgroundImage', 'url(' + url + ')'); App.elements.avatarPlaceholder.css('display', 'none');
                    } else { App.elements.avatar.css('backgroundImage', 'none'); App.elements.avatarPlaceholder.css('display', 'flex'); }
                },
                ...
                renderSurroundings: function(container, tagsStr) {
                    ...
                    if (tagsStr) {
                        tagsStr.split(/[,\，]/).forEach(function(tag) {
                            if (!tag.trim()) return;
                            var span = $('<span>').addClass('person-tag').text(tag.trim());
                            container.append(span);
                        });
                    } else { container.html('<span style="color:var(--color-text-light);font-style:italic;">暂无环境信息...</span>'); }
                },
                ...
                renderWorldInfo: function(world, wealth) {
                    ...
                    if (el.length) { el.css('display', hasAny ? '' : 'none'); }
                    if (!hasAny) return;
                ...
                renderWorldInfoWidget: function() {
                    ...
                    if (!top.length || !expand.length) return;
                    ...
                    top.html('<span class="wib-top-label">' + topItem.label + '</span>' +
                ...
                toggleWorldInfo: function() {
                    var el = App.elements.containers.worldInfo;
                    if (el.length) el.toggleClass('expanded');
                },

                renderInventory: function(container, inventoryData) {
                    container.empty();
                    if (!inventoryData || Object.keys(inventoryData).length === 0) {
                        container.html('<div style="color:var(--color-accent);font-size:0.85rem;grid-column:1/-1;text-align:center;font-style:italic;margin-top:20px;">物品栏目前为空...</div>'); return;
                    }
                    Object.keys(inventoryData).forEach(function(name) {
                        var count = inventoryData[name];
                        var item = $('<div>').addClass('inventory-item').attr('data-key', name);
                        item.html('<div class="inventory-name">' + name + '</div>');
                ...
                renderStagePanel: function() {
                    ...
                    if (!view.showStage) {
                        ...
                        taskPanel.css('display', 'none');
                        App.elements.containers.headerBtnArea.empty();
                        App.elements.containers.headerBtnArea.css('display', 'none');
                        App.elements.containers.userHeader.removeClass('no-border');
                        return;
                    }
                    /* 剧情模式：恢复显示 */
                    toggleArea.css('display', '');
                    taskPanel.css('display', '');
                    ...
                    if (nextStage || currentStage) {
                        if (isCollapsed) {
                            /* 折叠状态：按钮移到用户信息栏右侧 */
                            headerBtn.html('<span id="stage-collapse-toggle-btn" class="square-btn">\u5C55\u5F00<br>\u5267\u60C5</span>');
                            headerBtn.css('display', '');
                            toggleArea.empty();
                            userHeader.addClass('no-border');
                        } else {
                            /* 展开状态：按钮在原位（虚线下方） */
                            headerBtn.empty();
                            ...
                            userHeader.removeClass('no-border');
                        }
                        var collapseToggleBtn = $('#stage-collapse-toggle-btn');
                        if (collapseToggleBtn.length) {
                            collapseToggleBtn.on('click', function() { App.uiStateConfig.toggleTaskPanel(); });
                        }
                        var editBtn = $('#stage-edit-btn');
                        if (editBtn.length) {
                            editBtn.on('click', function() { App.actions.toggleEditMode(); });
                        }
                        var saveBtn = $('#stage-save-btn');
                        if (saveBtn.length) {
                            saveBtn.on('click', function() { App.actions.saveEdit(); });
                        }
                    } else {
                        toggleArea.empty();
                        headerBtn.empty();
                    ...
                    if (!nextStage && !currentStage) {
                        if (!isCollapsed) {
                            taskPanel.html('<div class="task-container"><div class="task-header">\u2726 \u5267\u60C5\u72B6\u6001 \u2726</div><div style="text-align: center; color: var(--color-accent); padding: 1.5rem; font-style:italic;">暂未开始剧情，若你正在进行自定义生成，可以在正则中暂时关闭状态栏显示</div></div>');
                        }
                        return;
                    }
                    ...
                    /* 展开状态：正常渲染 */
                    var container = $('<div>').addClass('task-container');
                    ...
                    container.html(html);
                    taskPanel.append(container);

                    var toggleBtn = $('#toggle-stage-view');
                    if (toggleBtn.length) {
                        toggleBtn.on('click', function() { App.uiStateConfig.toggleStage(); });
                    }
                },

                renderBodyState: function(statesStr) {
                    var container = App.elements.text.tagContainer;
                    container.find('.body-state-tag').remove();
                    if (!statesStr) return;
                    statesStr.split(/[\u3001,\uFF0C]/).map(function(s) { return s.trim(); }).filter(function(s) { return s; }).forEach(function(s) {
                        var tag = $('<div>').addClass('body-state-tag').text(s);
                ...
                updateAll: function() {
                    ...
                    if (!d || !d.user) return;
                    requestAnimationFrame(function() {
                        if (d.user.identity !== prev.title) { text.title.text(d.user.identity || '...'); prev.title = d.user.identity; }
                        if (d.user.bodyState !== prev.bodyState) { App.ui.renderBodyState(d.user.bodyState); prev.bodyState = d.user.bodyState; }
                        if (d.user.psyche !== prev.psyche) { text.psyche.text(d.user.psyche || '没有特殊的情绪波澜...'); prev.psyche = d.user.psyche; }
            ...
            actions: {
                turnPage: function() {
                    ...
                    var current = null;
                    actives.forEach(function(p) { if ($(p).hasClass('active')) current = p; });
                ...
                showPage: function(pageEl) {
                    if (!pageEl) return;
                    var $page = $(pageEl);
                    App.elements.containers.pages.removeClass('active');
                    $page.addClass('active');
                    var isFirst = ($page.get(0) === App.elements.containers.pages.get(0));
                    var settingsToggle = App.elements.settings.toggle;
                    if (isFirst) {
                        settingsToggle.css('opacity', '1'); settingsToggle.css('pointerEvents', 'auto');
                    } else {
                        settingsToggle.css('opacity', '0'); settingsToggle.css('pointerEvents', 'none');
                    }
                },
                ...
                refreshPages: function() {
                    ...
                    pages.each(function() { if ($(this).hasClass('active')) currentActive = this; });

                    var actives = [pages.get(0)]; /* 第 1 页（个人信息）始终激活 */
                    /* 第 2 页：剧情（剧情模式）或世界观第一页有区块 */
                    if (view.showStage || (wp[0] && wp[0].length)) actives.push(pages.get(1));
                    for (var i = 2; i < pages.length; i++) {
                        var wpi = i - 1;
                        if (wp[wpi] && wp[wpi].length) actives.push(pages.get(i));
                    }
                ...
                openItemDetail: function(name, data) {
                    ...
                    App.elements.itemDetail.nameElem.text(name);
                    var descStr = '';
                    ...
                    if(App.elements.itemDetail.descElem.length) {
                       App.elements.itemDetail.descElem.text(descStr || "暂无特别需要留意的详细描述。");
                    }
                    App.elements.itemDetail.panel.addClass('active');
                },
                closeItemDetail: function() {
                    App.elements.itemDetail.panel.removeClass('active');
                },

                deleteInventoryItem: function(itemName) {
                    App.state.pendingDeleteItem = itemName;
                    App.elements.deleteConfirm.msgContent.text('确认操作：你确定要彻底丢弃物品 [ ' + itemName + ' ] 吗？\n一旦删除，该操作将无法撤回。');
                    App.elements.deleteConfirm.panel.addClass('active');
                },
                confirmDelete: async function() {
                    ...
                    try {
                        if (window.eventEmit) {
                            ...
                            var itemEl = $('.inventory-item[data-key="' + itemName.replace(/"/g, '\\"') + '"]');
                            if (itemEl.length) { itemEl.css('transition', 'all 0.2s'); itemEl.css('opacity', '0'); itemEl.css('transform', 'scale(0.8)'); setTimeout(function() { itemEl.remove(); }, 200); }
                        } else {
                            alert('\u65E0\u6CD5\u6B63\u5E38\u63A5\u5165SillyTavern\u7684\u6307\u4EE4\u901A\u9053\u3002');
                        }
                    } catch (e) { alert('\u5220\u9664\u64CD\u4F5C\u53D1\u751F\u672A\u77E5\u9519\u8BEF\uFF0C\u8BF7\u91CD\u8BD5\u3002'); }
                    App.elements.deleteConfirm.panel.removeClass('active'); App.state.pendingDeleteItem = null;
                },
                cancelDelete: function() { App.elements.deleteConfirm.panel.removeClass('active'); App.state.pendingDeleteItem = null; },
                ...
                saveEdit: async function() {
                    ...
                    var payload = { '\u5267\u60C5\u7EBF': {} };
                    var editables = $('.stage-text[contenteditable="true"]');
                    editables.each(function() {
                        var el = $(this);
                        var key = el.attr('data-stage-key');
                ...
                _getEditSnapshot: function() {
                    ...
                    editables.each(function() {
                        ...
                        snapshot[key][field] = el.text().trim();
                    });
                ...
                getVariableData: async function() {
                    try {
                        if (typeof getAllVariables === 'function') {
                            var raw = _.get(getAllVariables(), 'stat_data');
                            if (typeof raw === 'string') { raw = JSON.parse(raw); }
                            return raw || {};
                        }
                        /* 非宿主环境：使用 demo 数据本地预览 */
            ...
            bindEvents: function() {
                ...
                function toggleModal(panel) { panel.toggleClass('active'); }
                ...
                collapsePanelBtn.on('click', function(e) {
                    e.stopPropagation();
                    App.uiStateConfig.togglePanel();
                });

                statusCard.on('click', function() {
                    if (App.uiStateConfig.data.isPanelCollapsed) {
                        App.uiStateConfig.togglePanel();
                    }
                });

                deleteConfirm.btnConfirm.on('click', App.actions.confirmDelete);
                deleteConfirm.btnCancel.on('click', App.actions.cancelDelete);

                itemDetail.btnClose.on('click', App.actions.closeItemDetail);
                itemDetail.btnDelete.on('click', function() {
                    App.actions.closeItemDetail();
                    App.actions.deleteInventoryItem(App.state.pendingDeleteItem);
                });

                pageTurnBtn.on('click', App.actions.turnPage);

                /* 世界信息条：点击置顶项展开/收起，选择器里改置顶项 */
                if (App.elements.containers.worldInfoTop.length) {
                    App.elements.containers.worldInfoTop.on('click', function() {
                        App.ui.toggleWorldInfo();
                    });
                }
                if (App.elements.containers.worldInfoExpand.length) {
                    App.elements.containers.worldInfoExpand.on('change', function(e) {
                ...
                Object.keys(settings.sliders).forEach(function(k) { if(settings.sliders[k].length) settings.sliders[k].on('input', App.throttle(function() { App.settings.save(); }, 100)); });
                settings.fontSelector.on('change', function() { App.settings.save(); });

                settings.buttons.reset.on('click', function() { App.settings.reset(); });

                avatar.on('click', function() {
                    var input = $('<input>').attr('type', 'file').attr('accept', 'image/*');
                ...
                $(window).on('beforeunload', function() { if (App.state.currentAvatarUrl) URL.revokeObjectURL(App.state.currentAvatarUrl); });
            },
